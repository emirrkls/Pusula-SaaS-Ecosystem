...
public class GooglePlayVerificationServiceImpl implements GooglePlayVerificationService {
    ...
    public GoogleVerificationResult verifySubscription(String purchaseToken, String productId) {
        if (packageName == null || packageName.isBlank() || apiAccessToken == null || apiAccessToken.isBlank()) {
            return new GoogleVerificationResult(false, null, null, "Google Play doğrulama konfigürasyonu eksik");
        }
        ...
        try {
            ...
            if (body == null) {
                return new GoogleVerificationResult(false, null, null, "Google API boş yanıt döndü");
            }
            ...
            if (expiry == null || orderId == null) {
                return new GoogleVerificationResult(false, null, null, "Satın alma kaydı geçersiz");
            }
            if (paymentState != null && !"1".equals(String.valueOf(paymentState))) {
                return new GoogleVerificationResult(false, null, null, "Ödeme state uygun değil");
            }
            ...
            return new GoogleVerificationResult(true, String.valueOf(orderId), productId, null);
        } catch (RestClientException ex) {
            log.warn("Google Play verify failed: {}", ex.getMessage());
            return new GoogleVerificationResult(false, null, null, "Google doğrulama başarısız");
        }
    }
}
