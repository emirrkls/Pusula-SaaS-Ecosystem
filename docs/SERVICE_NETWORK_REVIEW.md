...
- Yönetici bildiriminden ağ ekranına geçiş. Push iletim altyapısı mevcut APNs yapılandırmasını kullanır; çevrimiçi iletim ayrı cihaz kabul testi gerektirir.
...
- Backend tam `mvn verify`: 214 test, 0 hata, 1 atlama. Atlanan PostgreSQL migration testi ayrı PostgreSQL koşusunda çalıştırıldı.
- Servis Ağı entegrasyonları: 22 test, H2 ve PostgreSQL 17'de başarılı. Tenant izolasyonu, rol, davet onayı, döngü/ikinci kademe engeli, şifre hash'i, limitler, tarih/arama, transaction geri alma, özel veri sızıntısı, 400 kayıt, eşzamanlı kabul ve kota yarışı dahil.
- Gerçek V35 SQL migration: PostgreSQL 17'de tekillik, partial index, kabul-fiș bağlantısı ve randevu kontrolleri başarılı (toplam 23 PostgreSQL testi).
- Masaüstü tam `mvn verify`: mevcut 27 test başarılı. Ek JavaFX testi localhost sentetik verilerle 900×600 alanda liste/form ve görünür aksiyonları render ederek sınar; canlı API kullanmaz.
...
7. Hesap oluşturma sonrası güvenli tek kullanımlık davet/şifre belirleme bağlantısı; bugün yönetici ilk şifreyi belirleyip güvenli kanaldan paylaşır. Oluşturma yanıtı bağlantı nedeniyle kaybolursa yeniden oluşturmadan önce alt servis listesi kontrol edilmelidir.
...
2. Üretim PostgreSQL'in tam yedeğini al ve ayrı restore ile doğrula. Önce backend V35 migration'ını ve uygulamayı yayınla; hiçbir işletmenin ağ yetkisini topluca açma.
